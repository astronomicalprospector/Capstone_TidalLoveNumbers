...
class TOVSolver:
    ...
    def __init__(self, nb, ed, p):

        self.nb = nb
        self.ed = ed
        self.p = p

        self.EOS_internal_interpolator = interp1d(self.p, self.nb, kind='linear', fill_value='extrapolate')
        self.EOS_interpolator_type2 = interp1d(self.nb, self.ed, kind='linear', fill_value='extrapolate')

    def solve_tov(self, central_pressure):
        def findED(p):
            return self.EOS_interpolator_type2(self.EOS_internal_interpolator(p))
        ...
        tov_solutions = solve_ivp(dtdr, (self.dr, self.r_max), init_cond,
                   events = stop_condition, dense_output=True, max_step = self.dr,
        ...
        ax2 = ax1.twinx()
        color2 = 'tab:red'
        ax2.set_ylabel(r'Mass ($M_\odot$)', color=color2)
