...
output_filename = "AnalyticSolution_TolmanVII.csv"

df = pd.DataFrame(columns=['compactness', 'k2'])
existing_indices = []

#Analytic Functions (Tolman VII Solution)
def EOS_ratio(beta, x):
    return (2 / 15) * np.sqrt(3 / (beta * exp_lambda(beta, x))) * np.tan(phi(beta, x)) - (1 / 3) + (x / 5)

def phi(beta, x):
    w_one = np.log((1/6) + np.sqrt((1 - 2 * beta)/(3 * beta)))
    w = np.log((x - (5 / 6) + np.sqrt((exp_lambda(beta, x))**(-1) / (3 * beta))))
    phi_one = np.arctan(np.sqrt(beta / (3 * (1 - 2 * beta))))

    return ((w_one - w) / 2) + phi_one

def squared_sound_speed(beta, x):
    return (np.tan(phi(beta, x)) / 5) * (np.tan(phi(beta, x) +
                                                np.sqrt(beta / (3 * exp_lambda(beta, x))) * (5 - 6 * x)))

#metric functions (used in differentials)

def exp_lambda(beta, x): # dimensionless
    return 1 / (1 - beta * x * (5 - 3 *x))

def v_prime(alpha, beta, x): # dimensionless
    return exp_lambda(beta, x) * ( beta * x * (5 - 3 *x) + 2 * alpha  * x * EOS_ratio(beta, x) )

#Q function
def Q_prime(alpha, beta, x): # dimensionless
    return ((alpha * x * exp_lambda(beta, x) * (5* (1 - x) + 9*EOS_ratio(beta, x) +
                                            (1 - x + EOS_ratio(beta, x)) / (squared_sound_speed(beta, x)))
...
for j, n in enumerate(compactness):
    ...
    def dydx(x, y):  # km^-1
        rhs = -((y ** 2 + y) + y * x * exp_lambda(n, x) *
                                              ( n * (5 - 3 * x) + alpha * (EOS_ratio(n, x) + x - 1))
                                              + Q_prime(alpha, n, x))
    ...
    r = n * np.sqrt(y_solutions.t)

    """
    plt.figure()
    plt.plot(r, y_solutions.y[0])
